...
ENVIRONMENTS = [
    {
        ...
        "services": [
            ...
            "TOTVS-Appserver12-APEX-HML3-WF",
            "TOTVS-Appserver12-APEX-HML3-WS",
            "TOTVS-Appserver12-APEX-HML3-WS2",
        ],
    },
    {
        "name": "INFRA",
...
def status():
    ...
    for env in ENVIRONMENTS:
        ...
        for service_name in env["services"]:
            services.append({"name": service_name, "status": get_service_status(service_name)})

        result.append({"environment": env["name"], "services": services})
...
def action():
    ...
    if not service or not action_type:
        return jsonify({"success": False, "error": "Serviço e ação são obrigatórios."}), 400
    ...
    try:
        ...
        save_log(service, action_type, "SUCCESS", user["username"])
        return jsonify({"success": True})
    except Exception as exc:
        msg = f"ERRO: {service} - {action_type}"
        save_log(service, action_type, "ERROR", user["username"])
        send_teams(msg)
        send_email(msg)
...
def update_user(username):
    ...
    for user in users_data:
        ...
        active_admins = sum(
            1 for item in simulated_users if item.get("role") == "admin" and item.get("active", True)
        )
        if active_admins == 0:
